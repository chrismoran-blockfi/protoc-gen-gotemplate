...
func extractAPIOptions(meth *descriptor.MethodDescriptorProto) (*options.HttpRule, error) {
	...
	eType, err := protoregistry.GlobalTypes.FindExtensionByNumber(proto.MessageName(meth.Options), protoreflect.FieldNumber(options.E_Http.Field))
	if err != nil {
		return nil, err
	}
	ext := proto.GetExtension(meth.Options, eType)
	if err != nil {
		return nil, err
	}
